...
public class LuauParser implements PsiParser, LightPsiParser {
  ...
  private static boolean BoundTypeList_0_0(PsiBuilder builder_, int level_) {
    ...
    parseTokens(builder_, 0, ID, COLON);
    return true;
  }
  ...
  /* ********************************************************** */
  // INTERP_BEGIN expression { INTERP_MID expression } INTERP_END
  public static boolean stringinterp(PsiBuilder builder_, int level_) {
    if (!recursion_guard_(builder_, level_, "stringinterp")) return false;
    if (!nextTokenIs(builder_, INTERP_BEGIN)) return false;
    boolean result_;
    Marker marker_ = enter_section_(builder_);
    result_ = consumeToken(builder_, INTERP_BEGIN);
    result_ = result_ && expression(builder_, level_ + 1, -1);
    result_ = result_ && stringinterp_2(builder_, level_ + 1);
    result_ = result_ && consumeToken(builder_, INTERP_END);
    exit_section_(builder_, marker_, STRINGINTERP, result_);
    return result_;
  }

  // INTERP_MID expression
  private static boolean stringinterp_2(PsiBuilder builder_, int level_) {
    if (!recursion_guard_(builder_, level_, "stringinterp_2")) return false;
    boolean result_;
    Marker marker_ = enter_section_(builder_);
    result_ = consumeToken(builder_, INTERP_MID);
    result_ = result_ && expression(builder_, level_ + 1, -1);
    exit_section_(builder_, marker_, null, result_);
    return result_;
  }
  ...
  // NUMBER | STRING | 'nil' | 'true' | 'false' | '...' | tableconstructor | closureExpr | prefixExp | ifelseexp | stringinterp
  public static boolean simpleExp(PsiBuilder builder_, int level_) {
    ...
    if (!result_) result_ = stringinterp(builder_, level_ + 1);
    exit_section_(builder_, level_, marker_, result_, false, null);
    return result_;
  }
